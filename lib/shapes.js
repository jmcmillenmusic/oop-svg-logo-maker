// This function creates the canvas, the shape for the logo, and the letter to go on the logo based on user input.
function generateShape(data) {
    ...
    // This creates the LogoText class which contains the X and Y coordinates, font size, text-anchor, and its color. This also receives the letters and letter color from the user input.
    class LogoText {
    ...
    // This constructs the text and adds it to the logo.
    const newLogoText = new LogoText("150", "125", "60", "middle", `${data.charactersColor}`);
    newShape += newLogoText.renderText();

    // This returns the logo as a whole once all of its parts have been rendered.
    return newShape;
}
